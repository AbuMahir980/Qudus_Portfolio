...
const BlueprintTerminal: React.FC<BlueprintTerminalProps> = ({ blueprint, className }) => {
    return (
        <motion.div
            ...
            viewport={{ once: true }}
            className={cn(
                "bg-zinc-950 border border-zinc-800 rounded-lg overflow-hidden font-mono text-sm shadow-2xl",
                className
            )}
        >
            ...
            <div className="p-4 overflow-x-auto">
                <div className="flex gap-4 mb-4 border-b border-zinc-800/50 pb-2">
                    <span className="text-cyan-400/80 leading-none">TYPE: {blueprint.type.toUpperCase()}</span>
                    <span className="text-zinc-600 leading-none">|</span>
                    <span className="text-purple-400/80 leading-none">LANG: {blueprint.language.toUpperCase()}</span>
                </div>
                <pre className="text-zinc-300 leading-relaxed whitespace-pre font-mono">
                    <code>{blueprint.code}</code>
                </pre>
            </div>
            <div className="bg-zinc-900/30 px-4 py-1.5 border-t border-zinc-800/50">
